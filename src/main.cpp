...
#if defined(NDEBUG)
# error "Ahricoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xd53142975b8b85b3823d91949f1dd059dab91f992d3ea3c91f83411482d4c0af");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Ahricoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Ahricoin Signed Message:\n";
...
{
    // Ahricoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Ahricoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Ahricoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1472922008, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "Ahricoin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04ae94a59eccc7d82e1aa21a18e9875198cf24e66818cd76b9db99a6a75de9bf21b26a1c6160aafbe3b8b3329704b30b4a7ff5e936879d991e19e2cb1f2c5b4dae") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1472922008;
        ...
        assert(block.hashMerkleRoot == uint256("0x815d71ac56fba91fa1681effe02c67f4a762ef422a2fe10e769bc2bf07ebe039"));
...
unsigned char pchMessageStart[4] = { 0x35, 0x87, 0x8f, 0xdf }; // Ahricoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("AhricoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("AhricoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static AhricoinMiner(CWallet *pwallet)
{
    printf("AhricoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("ahricoin-miner");
    ...
    try { loop {
        ...
        printf("Running AhricoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("AhricoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&AhricoinMiner, pwallet));
}
